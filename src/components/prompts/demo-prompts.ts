import type { Prompt } from "@prisma/client";

type DemoPrompt = Pick<Prompt, "id" | "title" | "tags" | "body" | "pinnedAt">;

export const DEMO_PROMPTS: DemoPrompt[] = [
  {
    id: "demo-bug-triage",
    title: "BUG切り分けテンプレ（最初の10分）",
    tags: ["demo", "bug", "triage"],
    body: [
      "以下の不具合を、最初の10分で切り分けして。",
      "",
      "## ゴール",
      "{{goal_text}}",
      "",
      "## 現象",
      "{{current_text}}",
      "",
      "## 期待する挙動",
      "{{acceptance_details}}",
      "",
      "## エラーログ",
      "{{error_logs}}",
      "",
      "## 追加情報",
      "### 環境",
      "{{env}}",
      "",
      "### 優先度",
      "{{priority}}",
      "",
      "### 直前に変えた点",
      "{{change_text}}",
      "",
      "### 再現手順",
      "{{steps_text}}",
    ].join("\n"),
    pinnedAt: null,
  },

  {
    id: "demo-e2e-fail",
    title: "Playwright E2E失敗の調査テンプレ",
    tags: ["demo", "e2e", "playwright"],
    body: [
      "以下のPlaywright E2E失敗を調査して、最小修正方針を出して。",
      "",
      "## 失敗概要",
      "{{current_text}}",
      "",
      "## 失敗ログ（重要部分だけ）",
      "{{error_logs}}",
      "",
      "## 直近変更",
      "{{change_text}}",
      "",
      "## 実行環境",
      "{{env}}",
      "",
      "## 期待する修正方針",
      "{{goal_text}}",
      "",
      "## チェックリスト",
      "- selector は data-pv / getByTestId に寄せたか",
      "- 文言依存になっていないか",
      "- waitForTimeout で誤魔化していないか",
    ].join("\n"),
    pinnedAt: null,
  },

  {
    id: "demo-prisma-migration-investigation",
    title: "Prisma / migration失敗調査テンプレ",
    tags: ["demo", "prisma", "migration"],
    body: [
      "以下のPrisma / migration失敗を調査して、修正方針を整理して。",
      "",
      "## ゴール",
      "{{goal}}",
      "",
      "## 現象",
      "{{symptom_text}}",
      "",
      "## エラーログ（重要部分）",
      "{{error_logs}}",
      "",
      "## 直前変更",
      "{{change_text}}",
      "",
      "## 実行したコマンドと結果",
      "{{steps_text}}",
      "",
      "## 修正方針",
      "{{fix_plan_text}}",
      "",
      "## 追加情報",
      "### 確認環境",
      "{{env}}",
      "",
      "### 期待する状態",
      "{{acceptance_details}}",
    ].join("\n"),
    pinnedAt: null,
  },

  {
    id: "demo-post-deploy-check",
    title: "デプロイ後確認テンプレ（反映確認・切り分け）",
    tags: ["demo", "deploy", "verification"],
    body: [
      "以下のデプロイ後確認を進めて、問題時の切り分けと判断材料を整理して。",
      "",
      "## 目的",
      "{{goal}}",
      "",
      "## 変更内容",
      "{{change_text}}",
      "",
      "## 確認環境",
      "{{env}}",
      "",
      "## 確認項目",
      "{{checks_details}}",
      "",
      "## 問題時の切り分け",
      "{{triage_text}}",
      "",
      "## ロールバック判断メモ",
      "### 影響範囲",
      "{{impact_text}}",
      "",
      "### 暫定対応",
      "{{mitigation_text}}",
      "",
      "### 判断条件",
      "{{rollback_criteria_text}}",
    ].join("\n"),
    pinnedAt: null,
  },

  {
    id: "demo-postmortem-organize",
    title: "ポストモーテム整理テンプレ（原因・対応・再発防止）",
    tags: ["demo", "incident", "postmortem"],
    body: [
      "以下の事象について、ポストモーテムを整理して。",
      "",
      "## 目的",
      "{{goal}}",
      "",
      "## 何が起きたか（時系列）",
      "{{timeline_text}}",
      "",
      "## 原因（事実 / 推測を分ける）",
      "{{cause_text}}",
      "",
      "## 対応（実施内容）",
      "{{action_text}}",
      "",
      "## 再発防止",
      "{{prevention_text}}",
      "",
      "## 共有メモ",
      "### 影響範囲",
      "{{impact}}",
      "",
      "### 検知方法",
      "{{detection}}",
      "",
      "### 恒久対応の担当/期限",
      "{{owner}}",
    ].join("\n"),
    pinnedAt: null,
  },

  {
    id: "demo-codex-impl-request",
    title: "Codex依頼：小さく安全に実装（E2E込み）",
    tags: ["demo", "codex", "dev"],
    body: [
      "以下の改善タスクを「小さく安全に」実装して。",
      "",
      "## Goal",
      "{{goal_text}}",
      "",
      "## Acceptance（期待する状態）",
      "{{acceptance_details}}",
      "",
      "## Current（現状）",
      "{{current_text}}",
      "",
      "## Constraints（制約）",
      "{{constraints_details}}",
      "",
      "## Attachments（関連ファイル/ログ）",
      "{{code_text}}",
      "",
      "## Process（手順）",
      "1. 変更点を最小差分に分割して提案してから着手",
      "2. 実装 → `pnpm biome:check` → `pnpm build`",
      "3. E2E（Playwright）を含めた回帰確認",
      "4. 落ちたら直して通すまで",
      "5. 変更理由・影響範囲・テスト観点をPR本文用に要約",
    ].join("\n"),
    pinnedAt: null,
  },

  {
    id: "demo-playwright-e2e-add-request",
    title: "Playwrightテスト追加依頼テンプレ（新規E2E）",
    tags: ["demo", "playwright", "e2e"],
    body: [
      "以下の導線を守るPlaywrightテストを追加して。",
      "",
      "## 目的",
      "{{goal}}",
      "",
      "## 守りたい導線（ユーザー操作）",
      "{{flow_text}}",
      "",
      "## 受け入れ条件",
      "{{acceptance_details}}",
      "",
      "## テスト方針",
      "{{test_strategy_text}}",
      "",
      "## 確認方法",
      "{{steps_text}}",
      "",
      "## 実装時の制約",
      "{{constraints_details}}",
    ].join("\n"),
    pinnedAt: null,
  },

  {
    id: "demo-pr-review",
    title: "PRレビュー依頼（最短で通す）",
    tags: ["demo", "pr", "review"],
    body: [
      "以下の変更をレビューして。",
      "",
      "## 目的",
      "{{goal_text}}",
      "",
      "## 変更内容（要約）",
      "{{current_text}}",
      "",
      "## 受け入れ条件",
      "{{acceptance_details}}",
      "",
      "## 影響範囲（変更ファイル/導線）",
      "{{constraints_details}}",
      "",
      "## レビュー観点",
      "- 仕様と整合しているか",
      "- 例外/エラー時の挙動",
      "- テストの更新漏れ",
    ].join("\n"),
    pinnedAt: null,
  },

  {
    id: "demo-dependabot-grouped",
    title: "Dependabot（Grouped）対応：安全にまとめて上げる",
    tags: ["demo", "deps", "dependabot"],
    body: [
      "以下のDependabot更新を、安全にまとめて対応して。",
      "",
      "## ゴール",
      "{{goal_text}}",
      "",
      "## 現状",
      "{{current_text}}",
      "",
      "## 受け入れ条件（Acceptance）",
      "{{acceptance_details}}",
      "",
      "## 制約",
      "{{constraints_details}}",
      "",
      "## 手順（プロトコル）",
      "1. 影響範囲を洗い出す（lockfile / runtime deps / dev deps）",
      "2. まず security updates（grouped）を適用し、PR数を抑える",
      "3. `pnpm install` → `pnpm biome:check` → `pnpm build` → `pnpm test:e2e` を通す",
      "4. 失敗したら原因を切り分け、最小差分で修正して通すまで",
      "5. PR本文に「更新理由 / 影響範囲 / 回帰確認」を明記する",
      "",
      "## エラーログ",
      "{{error_logs}}",
    ].join("\n"),
    pinnedAt: null,
  },

  {
    id: "demo-config-where-and-how",
    title: "CONFIG：キー/設定どこ？（場所・手順）",
    tags: ["config", "setup", "env"],
    body: [
      "{{service}} で {{item}} の「場所」と「正しい設定手順」が分からない。",
      "省略せずに、クリック手順/画面遷移/入力例まで具体的に案内して。",
      "",
      "# 1) 前提",
      "### サービス",
      "{{service}}（例：Supabase / Vercel / GitHub / Auth0 / Cloudflare / Stripe）",
      "",
      "### 何を探してる？",
      "{{item}}（例：API Key / Project URL / Webhook Secret / env var / callback URL / deploy key）",
      "",
      "### 目的",
      "{{purpose_text}}",
      "",
      "### 環境",
      "{{env}}",
      "",
      "### いま困ってること",
      "{{problem_text}}",
      "",
      "### 今見えている画面/メニュー",
      "{{screen_text}}（添付ファイルがあれば明記）",
      "",
      "# 2) 求める出力（この順番で）",
      "1. 「その設定がある場所」への最短手順（クリック手順を省略しない）",
      "2. そこで何を入力/コピペするか（サンプル値の形式、例も）",
      "3. それを「どこに置くべきか」（.env.local / Vercel Environment Variables / GitHub Secrets / 管理画面 等）",
      "4. 動作確認の方法（確認ポイント、簡単なテスト）",
      "5. よくある落とし穴（権限不足、間違ったキー種別、公開してはいけない情報、反映タイミング）",
      "",
      "# 3) 制約",
      "- 分からない情報があれば、最初に質問は「最大3つ」まで。",
      "- ただし推測できるなら質問なしで進めてOK。",
      "- セキュリティ注意：公開してはいけない値（秘密鍵/トークン等）がある場合は明示して。",
    ].join("\n"),
    pinnedAt: null,
  },
];
